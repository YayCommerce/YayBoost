    ...
    const FBT = {
        ...
        bindEvents: function() {
            ...
            $(document).on('click', '.yayboost-fbt-single-add', function() {
                ...
                $item.find('.yayboost-fbt-selectable').prop('checked', false).trigger('change');
            });
        ...
        calculateTotal: function() {
            ...
            $container.find('.yayboost-fbt-total-price').html(this.formatPrice(total));
        },
        ...
        formatPrice: function(price) {
            ...
            if (typeof wc_add_to_cart_params !== 'undefined' && typeof accounting !== 'undefined') {
                try {
                    return accounting.formatMoney(price, {
                        symbol: wc_add_to_cart_params.currency_format_symbol || '$',
                        format: wc_add_to_cart_params.currency_format || '%s%v',
                        decimal: wc_add_to_cart_params.currency_format_decimal_sep || '.',
                        thousand: wc_add_to_cart_params.currency_format_thousand_sep || ',',
                        precision: wc_add_to_cart_params.currency_format_num_decimals || 2
                    });
                } catch (e) {
                    // Fallback if accounting.js fails
                }
            }
        ...
        handleBatchAdd: function($button) {
            ...
            const currentProductId = parseInt($container.data('current-product')) || 0;
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $container.find('.yayboost-fbt-selectable:not(:disabled)').prop('checked', false);
                        self.calculateTotal();
                    } else {
                        self.showNotice(response.data.message || yayboostFBT.i18n.error, 'error');
                    }
                },
                error: function() {
        ...
        showNotice: function(message, type) {
            const $notice = $('<div class="yayboost-fbt-notice yayboost-fbt-notice-' + type + '">' + message + '</div>');
