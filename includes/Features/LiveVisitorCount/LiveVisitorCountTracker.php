...
class LiveVisitorCountTracker {
    ...
    public function count_active_visitors( int $page_id ): int {
        ...
        $count = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(*) FROM $table WHERE page_id = %d AND last_active >= %d", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                $page_id,
                $expired_time
            )
        );
    ...
    public function clean_up_expired_visitors( int $expired_time ): void {
        ...
        $wpdb->query(
            $wpdb->prepare(
                "DELETE FROM $table WHERE last_active < %d", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                $expired_time
            )
        );
    }
