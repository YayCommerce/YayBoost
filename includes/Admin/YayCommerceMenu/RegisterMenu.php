...
class RegisterMenu {
	...
	public function __construct() {
		...
		add_action( 'admin_menu', array( $this, 'add_placeholder_menu' ), 100 );
	...
	public function get_submenus() {
		...
		if ( ! empty( $licensing_plugins_yay_mail ) || ! empty( $licensing_plugins_yay_pricing ) || ! empty( $licensing_plugins_yay_currency ) || ! empty( $licensing_plugins_yay_swatches ) || ! empty( $licensing_plugins_yay_extra ) || ! empty( $licensing_plugins_yay_smtp ) || ! empty( $yay_licensing_plugins ) ) {
			$submenus['yaycommerce-licenses'] = array(
				...
				'capability'         => 'manage_options',
				'render_callback'    => array( '\YAYDP\Admin\YayCommerceMenu\LicensesMenu', 'render' ),
				'load_data_callback' => array( '\YAYDP\Admin\YayCommerceMenu\LicensesMenu', 'load_data' ),
			);
		}
		...
		$submenus['yaycommerce-other-plugins'] = array(
			...
			'capability'         => 'manage_options',
			'render_callback'    => array( '\YAYDP\Admin\YayCommerceMenu\OtherPluginsMenu', 'render' ),
			'load_data_callback' => array( '\YAYDP\Admin\YayCommerceMenu\OtherPluginsMenu', 'load_data' ),
		);
	...
	public function add_placeholder_menu() {
		...
		if ( ! $has_plugin_menu ) {
			add_submenu_page( 'yaycommerce', __( 'YayBoost', 'yay-customer-reviews-woocommerce' ), __( 'YayBoost', 'yay-customer-reviews-woocommerce' ), 'manage_woocommerce', 'yayboost', array( $this, 'render_placeholder_menu' ), 0 );
		}
	}
